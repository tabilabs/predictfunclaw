...
- `predict-account + ERC_MANDATED_*`（推荐的带资金交易路径）
  ...
  - 最小字段：`PREDICT_ENV`、`PREDICT_WALLET_MODE=predict-account`、`PREDICT_API_KEY`、`PREDICT_ACCOUNT_ADDRESS`、`PREDICT_PRIVY_PRIVATE_KEY`、`ERC_MANDATED_MCP_COMMAND`、`ERC_MANDATED_CHAIN_ID`、`ERC_MANDATED_VAULT_ADDRESS`、`ERC_MANDATED_VAULT_ASSET_ADDRESS`、`ERC_MANDATED_VAULT_AUTHORITY`、`ERC_MANDATED_CONTRACT_VERSION`。
