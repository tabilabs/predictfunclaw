...
def test_wallet_status_mandated_vault_predicts_address_when_not_explicit() -> None:
    ...
    assert preview["predictedVault"] == payload["vaultAddress"]
    assert preview["signerAddress"] == Account.from_key(EOA_PRIVATE_KEY).address
...
def test_wallet_bootstrap_executes_and_returns_manual_env_payload() -> None:
    ...
    assert backfill_env["ERC_MANDATED_VAULT_ADDRESS"] == payload["deployedVault"]
    assert (
        backfill_env["ERC_MANDATED_FACTORY_ADDRESS"]
...
def test_wallet_status_overlay_can_resolve_asset_and_authority_from_vault_address(
...
def test_wallet_status_predict_account_overlay_requires_bootstrap_when_vault_is_undeployed() -> (
    None
):
    config = PredictConfig.from_env(
        {
    ...
    message = str(error.value)
    assert "predict-account + vault overlay needs a deployed vault first" in message
    assert "deploy or redeploy a vault" in message.lower()
